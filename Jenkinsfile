  ...
  stages{
    ...
    stage ('Nightly build') {
        ...
        stage('Nigthly build - build'){
          steps {
            ...
            checkout([
              ...
              userRemoteConfigs: [[credentialsId: '30bccc90-3e99-4958-b9a7-e822c26d0cdf', url: 'git@github.com:BeclsParticle/sciencemodule_UserInterface.git']]
            ])
      ...
      post {
        ...
        success {
          //trigger Deployment job when current job is successful
          build (
            job: 'Shepherd/ViabilityScienceModule-Deployment/master', 
            propagate: false, 
            wait: false,
            parameters: [[
      ...
      post {
        success {
          script {
            try {
              build (
                job: "Shepherd/ViabilityScienceModule-Deployment/${env.BRANCH_NAME}", 
                propagate: false, 
                wait: false,
                parameters: [
                  [$class: 'StringParameterValue',
