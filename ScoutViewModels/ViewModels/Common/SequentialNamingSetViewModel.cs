...
{
    public class SequentialNamingSetViewModel : BaseViewModel
    {
        #region Constructor

        public SequentialNamingSetViewModel()
        {
            TextFirst = true;
            SequentialNamingItems = new SequentialNamingItemViewModel[2];

            var vmText = new SequentialNamingItemViewModel(new SequentialNamingItem(
                new RunOptionSettingsModel().RunOptionsSettings.DefaultSampleId));
        ...
        {
            if (!string.IsNullOrEmpty(GetBaseString()))
            {
